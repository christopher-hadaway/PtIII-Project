function [lc_avg, lcArray] = findCapillaryLength_edits2(imgArray, folder, cr, cu, pxGrd)
...
% Returns the calculated capillary lengths and average capillary length
    ...
    initialCoeffs = [0,-7,0.0,0,1]; % initial start point for fitting
    ...
    for i = 1:num
        ...
        surfexp = fittype('(p4*exp(((p1*x)+(p2*y))))+sin(p3)*y+C', ...
                            'independent',{'x','y'}, ...
                            'dependent','h');
        ...
        lowerbounds = [-10,-20,-3,-0.5,0.5];%[0,-25,0.2,-0.1,-2,-1.5];
        upperbounds = [10,-0.1,3,0.5,2];%[2,10,2.0,0.1,3.0,1];
        
        [surffit,gof] = fit([xss,yss],hss,surfexp','Lower',lowerbounds,'StartPoint', initialCoeffs,'Upper',upperbounds)
        
        
        %plot(surffit,[xss(1:150:end),yss(1:150:end)],hss(1:150:end));%,hss(1:1000:end));%mesh(Xmesh(1:20:end,1:20:end),Ymesh(1:20:end,1:20:end),surffit);
        ...
        l2 = fittedCoeffs(3);
        lc = 1/(l1^2+l2^2)^(1/2);
        errorlc =lc* (abs(conLim(1,2)-l1))/l1;
    ...
    lcArray = 0.1068 * lcArray;

    %1080x2280 5.84" phone
    %lcArray = 0.0588 * lcArray;

    %1920p 0.39 inch
    %lcArray = 0.00458 * lcArray;
