% Set zero skew using Secant method
...
if sign(skewUpper) * sign(skewLower) == -1
    while ( abs(upper-lower)>2 ) && sign(skewUpper) * sign(skewLower) == -1
        midpoint_old = (upper + lower)/2

        midpoint = lower + skewLower * abs(lower - upper)/(skewUpper + skewLower)
